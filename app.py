...
.stButton > button:not([kind]):hover { opacity: .8 !important; }
...
with st.sidebar:
    ...
    send_strategy = st.selectbox(
        ...
        help="Каскад — пробует каналы по очереди\nОдин канал — строго один\nWA шаблон — только Химчистка"
    )

    st.markdown("---")

    cascade_order = []; target_source = "chat"; wa_template = ""
    ...
    if send_strategy in ("Каскад", "Один канал"):
        ...
        if send_strategy == "Каскад":
            ...
            target_source = st.selectbox(
                "Канал", ["chat", "tlgrm", "max"], format_func=lambda x: CHANNEL_LABELS[x]
            )
    else:
...
def send_himchistka_single(phone, msg, tv, tags, source):
    return _post(YANDEX_SINGLE_URL, {
        "phone": normalize_phone(phone), "message": msg, "type_value": tv,
        "tags": tags, "target_source": source
    })

def send_cleaning_hde(phone, msg, tv, tags, strategy, cascade_order=None, source=None):
    payload = {"phone": normalize_phone(phone), "message": msg, "type_value": tv, "tags": tags}
            ...
            if dry_run:
                ...
                if cascade_blocked:
                    ...
                    launch_clicked = launch_col.button(
                        "ЗАПУСТИТЬ РАССЫЛКУ", use_container_width=True, key="btn_bulk"
                    )
                    stop_col.button(
