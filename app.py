...
Отдельной стратегии «Один канал» нет: чтобы отправить строго в один канал,
оставь в «Приоритете каналов» единственный пункт.
...
Всё, что не сводится к `7XXXXXXXXXX`, отклоняется без запроса в HDE. Галка
«Стоп рассылка» на карточке клиента действует и здесь.

---

### Массовая рассылка

**Формат файла:** Excel (.xlsx) с номерами телефонов. Столбец определяется
автоматически — берётся тот, где больше всего значений похожи на телефон,
поэтому лишние колонки и заголовок не мешают. Под загрузчиком видно, какой
столбец распознан: сверься с этой подписью перед запуском.

**Отбраковка.** Годным считается только российский номер `7XXXXXXXXXX`. Всё
остальное отбрасывается до отправки и показывается таблицей. Если мусора
больше трети — почти наверняка взят не тот столбец или не тот файл, инструмент
скажет об этом отдельно. Пока не подтвердишь отбраковку галочкой, кнопка
запуска заблокирована.

> Почему так строго: поиск клиента в HDE работает по **префиксу**, а не по
> точному совпадению — `search=7` возвращает сотни чужих людей. Огрызок цифр
> находит постороннего абонента, и сообщение уходит ему. Так 25.08.2026 из-за
> столбца с email вместо телефонов (`novkot16@yandex.ru` → `16`) 477 сообщений
> ушло посторонним, один человек получил один и тот же текст 52 раза.

**Порядок действий:**
1. Загрузи файл
2. Проверь, какой столбец распознан и сколько строк отбраковано
3. Проверь параметры в блоке «Параметры запуска»
4. При необходимости включи **Сухой прогон** — покажет нормализованные номера без отправки
5. Нажми **Запустить рассылку**
6. Следи за лайв-логом
7. После завершения скачай отчёт

**Кнопка СТОП** — останавливает рассылку после текущего номера. Уже отправленные результаты сохранятся и будут показаны в отчёте. Кнопка и живой прогресс висят **вверху страницы** и видны из любой вкладки браузера, даже если файл в ней не загружали.

**Две рассылки разом не запустятся** — пока идёт одна, вторая отклоняется с ошибкой. Дождись конца или нажми СТОП.
...
| `Stop mailing flag (user field 22)` | Клиент отписан: на его карточке стоит галка «Стоп рассылка». Это не сбой — так и задумано, слать ему нельзя. Какая именно карточка — в логах Яндекс-функции (`SKIP stop-mailing`) |
| `Некорректный номер — не отправлено` | Номер не прошёл проверку `7XXXXXXXXXX`. Запрос в HDE не делался вовсе |
| `Invalid phone` | То же самое, но отбраковала Яндекс-функция — значит вызов пришёл мимо оркестратора (ноутбук, ручной запрос) |
| `HDE Send Error` | Ошибка API — проверь статус HDE |
| Ошибка сети | Проблема с Яндекс Клауд функцией |

---

### «Стоп рассылка» — как отписать клиента

В HDE на **карточке клиента** (не в заявке) есть галка «Стоп рассылка» —
пользовательское поле №22. Поставил — рассылки этому человеку больше не уходят.

Проверка живёт в Яндекс-функции, поэтому работает для **любого** способа
отправки, включая «Один номер» и запросы мимо оркестратора.

Один телефон в HDE — это несколько карточек, по одной на канал
(`@auto.tlgrm`, `@auto.max`, `@auto.vk` и основная). Галка **на любой из них**
блокирует отправку целиком, так что искать «правильную» карточку не нужно.
""")
